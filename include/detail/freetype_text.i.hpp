...
public:
    ...
    explicit font_cache_impl(font_libary& lib, string_ref filename, unsigned size);
...
private:
    static ft_unique<FT_Face> create_face_(FT_Library const lib) {
        //TODO

        FT_Face face {};
        auto const result = FT_New_Face(lib, R"(C:\windows\fonts\meiryo.ttf)", 0, &face);
        if (result) {
            BK_TODO_FAIL();
        }
...
font_cache_impl::font_cache_impl(
  ...
  : face_ {create_face_(lib.handle().as<FT_Library>())}
  , face_size_ {size}
{   
}
...
void font_cache_impl::chache(std::initializer_list<unicode::block_value> blocks) {
}
