...
font_face::get_texture(unicode::codepoint const cp) {
...
void
transitory_text_layout::render(renderer& r, font_face& face, int x, int y) {
    for (auto i = 0u; i < codepoints_.size(); ++i) {
        ...
        r.draw_texture(*info.t, info.r, renderer::rect {left, top, right, bottom});
    }
}
